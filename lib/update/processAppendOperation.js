...
function processAppendOperation({ resource, path, value, upsert }) {
    ...
    if (value instanceof Array) {
        const arrayTarget = ensureArrayValue_1.default(target);
        for (const each of value) {
            arrayTarget.push(each);
        }
    }
    else if (value instanceof Object) {
        const keyValueTarget = ensureKeyValue_1.default(target);
...
function resolveTarget(ensuredResource, path, initialValue) {
    ...
    while (currentPath.length > 0) {
        ...
        const parentOfTarget = ensureKeyValue_1.default(target);
        target = parentOfTarget[currentKey] = (_a = parentOfTarget[currentKey]) !== null && _a !== void 0 ? _a : currentInitialValue;
